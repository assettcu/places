...
<div class="search-results" style="margin-top:10px;">
	<?php foreach($places as $place): if(!$place->loaded) continue; ?>
	        		...
	        		<td class="lalign tvalign">
	        			<div class="place-info">
							<h1 style="margin-top:10px;margin-left:5px;">
								<?php echo $icon->render(); ?> <a href="<?=Yii::app()->createUrl('place');?>?id=<?=$place->placeid;?>"><?=$place->placename;?></a> <span style="font-weight:normal;font-size:12px;color:#ccc;">(<?=$place->placetype->singular;?>)</span>
							</h1>
							<div class="metadata" style="padding-left:15px;">
								<?php if(isset($place->parent_) and $place->parent_->placetype->machinecode=="building"): ?>
